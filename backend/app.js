...
app.get('/collection/:cid', function (req, res) {
    console.log("[200] " + req.method + " to " + req.url);
    mongo.query("artist", {cid: req.param.cid}, function(ret){
...
app.get('/api/collection/:cid', function (req, res) {
    console.log("[200] " + req.method + " to " + req.url);
    console.log(req.param.cid);
    mongo.query("artist", res, {cid: req.param.cid}, function(ret){
        res.json(ret);
    });
})

/* delete collection */
app.delete('/api/collection/', jsonParser, function (req, res) {
    if (!req.body) return res.sendStatus(400);
    console.log("[200] " + req.method + " to " + req.url);

    mongo.deletedoc("collection", req.body);
